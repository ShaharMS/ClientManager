...
public partial class ActiveAllocations
{
    ...
    private void HydrateFromUrl()
    {
        ...
        try
        {
            if (query.TryGetValue(QueryParamParsers.Pool, out var pool)
                && _poolOptions.Any(p => p.Id == pool))
            {
                _selectedPoolId = pool;
            }

            if (query.TryGetValue(QueryParamParsers.Metric, out var metric)
                && (metric == AllocationsChartSection.ActiveAllocationsMetric
                    || metric == AllocationsChartSection.AccessRequestsMetric))
            {
    ...
    private Dictionary<string, string?> BuildQueryParams()
    {
        ...
        if (_selectedPoolId != AllocationsLoadContext.AllPoolsId)
        {
            query[QueryParamParsers.Pool] = _selectedPoolId;
        }

        if (_selectedMetric != AllocationsChartSection.ActiveAllocationsMetric)
        {
            query[QueryParamParsers.Metric] = _selectedMetric;
        }
