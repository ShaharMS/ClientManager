...
public class DataSeedService : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        {
            var existing = await globalRateLimitDb.GetByIdAsync(globalLimit.Id, cancellationToken);
            if (existing is null)
            {
                await globalRateLimitDb.CreateAsync(globalLimit, cancellationToken);
        ...
        {
            var existing = await clientConfigDb.GetByIdAsync(config.Id, cancellationToken);
            if (existing is null)
            {
                await clientConfigDb.CreateAsync(config, cancellationToken);
