...
internal sealed class ClientConfigurationStoreClient(HttpClient httpClient) : IClientConfigurationStoreClient
{
    ...
    public Task<ServiceAccessSettings?> GetServiceSettingsAsync(string clientId, string serviceId, CancellationToken cancellationToken) =>
        GetOptionalForClientAsync<ServiceAccessSettings>(StorageApiRoutes.ClientConfigurations.ServiceSettings(clientId, serviceId), clientId, cancellationToken);
    ...
    public Task<ResourcePoolSettings?> GetResourcePoolSettingsAsync(string clientId, string poolId, CancellationToken cancellationToken) =>
        GetOptionalForClientAsync<ResourcePoolSettings>(StorageApiRoutes.ClientConfigurations.ResourcePoolSettings(clientId, poolId), clientId, cancellationToken);
    ...
    public Task<ClientRateLimit?> GetGlobalRateLimitAsync(string clientId, CancellationToken cancellationToken) =>
        GetOptionalForClientAsync<ClientRateLimit>(StorageApiRoutes.ClientConfigurations.GlobalRateLimit(clientId), clientId, cancellationToken);
    ...
    private async Task<T?> GetOptionalForClientAsync<T>(string path, string clientId, CancellationToken cancellationToken)
    {
        var response = await httpClient.GetAsync(path, cancellationToken);
        if (response.StatusCode == HttpStatusCode.NotFound)
        {
