...
using static ClientManager.Api.Storage.Stores.Implementations.Helpers.StoreSerialization;
using SearchDirection = ClientManager.Shared.Models.Search.SortDirection;
using MongoDB.Bson;
using MongoDB.Bson.Serialization;
using MongoDB.Driver;

namespace ClientManager.Api.Storage.Stores.Implementations;
...
public class MongoDBDocumentStore(IMongoDatabase database) : IDocumentStore
{
    ...
    {
        await EnsureTokenBucketStateMigratedAsync(tokensKey, lastRefillKey, cancellationToken);

        var filter = Builders<BsonDocument>.Filter.Eq("_id", tokensKey);
        var update = CreateTokenBucketConsumePipeline(
            bucketCapacity,
            tokensPerRefill,
        ...
        var result = await CounterCollection.FindOneAndUpdateAsync(filter, update, options, cancellationToken)
            ?? throw new InvalidOperationException($"Token bucket consume failed for '{tokensKey}'.");

        var lastRefill = ReadCounterField(result, "LastRefill");
        await SyncTokenBucketLastRefillMirrorAsync(lastRefillKey, lastRefill, cancellationToken);

        return (
            ReadCounterField(result, "TbAllowed") != 0,
            ReadCounterField(result, "TbRemaining"),
            ReadCounterField(result, "TbRetryAfter"));
    }
    ...
    private async Task EnsureTokenBucketStateMigratedAsync(
        string tokensKey,
        string lastRefillKey,
        CancellationToken cancellationToken)
    {
        ...
        var lastRefill = ReadCounterField(doc, "LastRefill");
        if (lastRefill != 0)
        {
            return;
        }

        var legacyRefill = await GetCounterAsync(lastRefillKey, cancellationToken);
        if (legacyRefill == 0)
        {
            return;
        }

        await CounterCollection.UpdateOneAsync(
            Builders<BsonDocument>.Filter.And(
                filter,
                Builders<BsonDocument>.Filter.Or(
                    Builders<BsonDocument>.Filter.Exists("LastRefill", false),
                    Builders<BsonDocument>.Filter.Eq("LastRefill", 0L))),
            Builders<BsonDocument>.Update.Set("LastRefill", legacyRefill),
            cancellationToken: cancellationToken);
    }

    private async Task SyncTokenBucketLastRefillMirrorAsync(
        string lastRefillKey,
        long lastRefill,
        CancellationToken cancellationToken)
    {
        var filter = Builders<BsonDocument>.Filter.Eq("_id", lastRefillKey);
        var update = Builders<BsonDocument>.Update
            .Max("Count", lastRefill)
            .Set("WindowStart", DateTime.UtcNow);
        await CounterCollection.UpdateOneAsync(
            filter,
            update,
    ...
    private static UpdateDefinition<BsonDocument> CreateTokenBucketConsumePipeline(
        int bucketCapacity,
        int tokensPerRefill,
        long refillIntervalSeconds,
        long nowUnixSeconds)
    {
        ...
        var lastRefillValue = new BsonDocument("$toLong", CreateIfNullExpression("$LastRefill", 0L));
        var storedTokens = new BsonDocument("$toLong", CreateIfNullExpression("$Count", 0L));
        var alignedNow = CreateAlignedWindowExpression(nowUnixSeconds, refillIntervalSeconds);
        var alignedLast = CreateAlignedWindowExpression(lastRefillValue, refillIntervalSeconds);
        var intervalsPassed = new BsonDocument("$toLong",
            new BsonDocument("$floor", new BsonDocument("$divide", new BsonArray
            {
                new BsonDocument("$subtract", new BsonArray { alignedNow, alignedLast }),
                refillIntervalSeconds
            })));
        var isInit = new BsonDocument("$eq", new BsonArray { lastRefillValue, 0L });
        var tokensToAdd = new BsonDocument("$multiply", new BsonArray { intervalsPassed, tokensPerRefill });
        var tokensAfterRefill = new BsonDocument("$cond", new BsonArray
        {
            isInit,
            bucketCapacity,
            new BsonDocument("$min", new BsonArray
            {
                bucketCapacity,
                new BsonDocument("$add", new BsonArray { storedTokens, tokensToAdd })
            })
        });
        var newLastRefill = new BsonDocument("$cond", new BsonArray
        {
            isInit,
            nowUnixSeconds,
            new BsonDocument("$cond", new BsonArray
            {
                new BsonDocument("$gt", new BsonArray { intervalsPassed, 0L }),
                alignedNow,
                lastRefillValue
            })
        });
        var isDenied = new BsonDocument("$and", new BsonArray
        {
            new BsonDocument("$not", isInit),
            new BsonDocument("$lte", new BsonArray { tokensAfterRefill, 0L })
        });
        var remaining = new BsonDocument("$cond", new BsonArray
        {
            isInit,
            initialTokens,
            new BsonDocument("$cond", new BsonArray
            {
                isDenied,
                0L,
                new BsonDocument("$subtract", new BsonArray { tokensAfterRefill, 1L })
            })
        });

        var stage = new BsonDocument("$set", new BsonDocument
        {
            { "Count", new BsonDocument("$cond", new BsonArray { isDenied, 0L, remaining }) },
            { "LastRefill", newLastRefill },
            { "WindowStart", new BsonDateTime(DateTime.UtcNow) },
            { "TbAllowed", new BsonDocument("$cond", new BsonArray { isDenied, 0L, 1L }) },
            { "TbRemaining", remaining },
            {
                "TbRetryAfter",
                new BsonDocument("$cond", new BsonArray
                {
                    ...
                    new BsonDocument("$max", new BsonArray
                    {
                        ...
                        {
                            refillIntervalSeconds,
                            new BsonDocument("$subtract", new BsonArray { nowUnixSeconds, alignedNow })
                        })
                    }),
                    0L
                })
    ...
    private static long ReadCounterField(BsonDocument document, string field) =>
        document.GetValue(field, 0).ToInt64();

    private static BsonDocument CreateIfNullExpression(string fieldPath, long defaultValue) =>
        new("$ifNull", new BsonArray { fieldPath, defaultValue });

    private static BsonDocument CreateAlignedWindowExpression(BsonValue timestamp, long intervalSeconds) =>
        new("$toLong", new BsonDocument("$multiply", new BsonArray
        {
            new BsonDocument("$floor", new BsonDocument("$divide", new BsonArray { timestamp, intervalSeconds })),
            intervalSeconds
        }));
}
